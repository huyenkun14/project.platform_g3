...
//    @Query("SELECT t FROM Transaction t WHERE t.category.user.id = :userId ORDER BY t.id")
    List<Transaction> findAllByCategoryUserIdOrderByIdDesc(Long userId);

    List<Transaction> findAllByCategoryIdOrderByIdDesc(Long categoryId);

    @Query("SELECT t FROM Transaction t WHERE t.category.user.id = :userId AND MONTH(t.date) = :month AND YEAR(t.date) = :year ORDER BY t.id DESC")
    ...
    @Query("SELECT t FROM Transaction t WHERE t.category.id = :categoryId AND MONTH(t.date) = :month AND YEAR(t.date) = :year ORDER BY t.id DESC")
